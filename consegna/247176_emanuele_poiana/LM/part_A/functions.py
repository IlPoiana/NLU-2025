...
def init_model(architecture = 'RNN', params = {
    ...
    if architecture == 'LSTM':
        ...
        model = LM_RNN(**params, output_size=VOCAB_LEN, pad_index=LANG.word2id["<pad>"]).to(DEVICE)
    elif architecture == 'DROPOUT':
        model = LSTM_Dropout(**params, output_size=VOCAB_LEN,pad_index=LANG.word2id["<pad>"]).to(DEVICE)
    return model
